...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const unsubscribe = authService.onAuthChange(user => {
        if (user) {
          setGoogleUser({ name: user.displayName || '', email: user.email || '', imageUrl: user.photoURL || '' });
          handleSync();
        } else {
          setGoogleUser(null);
        }
    ...
    loadInitialData();
  }, [handleSync, reloadDataFromStorage]);
  ...
  const appContentClass = isApiKeyModalOpen && !apiKey ? 'blur-sm pointer-events-none' : '';
      ...
      <div className={appContentClass}>
        ...
        <main className={mainContainerClass}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[24rem_minmax(0,1fr)_24rem] xl:grid-cols-[28rem_minmax(0,1fr)_28rem] lg:gap-8">
                ...
                <CharacterPrimaryPanel stats={cumulativeStats} isAnalyzing={isAnalyzing} onStatsChange={handleStatsChange} onDataLoaded={reloadDataFromStorage} />
                </aside>
                <div className="min-w-0">{renderMainContent()}</div>
                <aside className="hidden lg:block sticky top-8 self-start">
                <CharacterPanel stats={cumulativeStats} isAnalyzing={isAnalyzing} isOpen={true} onClose={() => {}} isSidebar={true} onStatsChange={handleStatsChange} onDataLoaded={reloadDataFromStorage} />
                </aside>
            </div>
            ) : (
        ...
        <div className="lg:hidden">
            {isReading && (
                <>
                    ...
                    <CharacterPanel isOpen={isPanelVisible} onClose={() => setIsPanelVisible(false)} stats={cumulativeStats} isAnalyzing={isAnalyzing} isSidebar={false} onStatsChange={handleStatsChange} onDataLoaded={reloadDataFromStorage} />
                </>
            )}
        </div>
        {isReading && (
