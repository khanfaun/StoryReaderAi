    ...
    if (status === 'idle' || status === 'error') {
        return (
             <div className="p-4 bg-[var(--theme-bg-base)] border border-dashed border-[var(--theme-border)] rounded-lg shadow-md flex flex-col items-center justify-center min-h-[120px]">
                <button onClick={handlePlayPause} className="w-24 h-24 flex items-center justify-center bg-purple-600 hover:bg-purple-700 text-white rounded-full text-2xl transition-transform transform hover:scale-105 disabled:bg-slate-600">
                    {playButtonContent()}
                </button>
        ...
        <div className="p-4 bg-[var(--theme-bg-base)] border border-[var(--theme-border)] rounded-lg shadow-md space-y-3">
            <div className="flex justify-between items-center gap-4">
                <div className="flex items-center gap-2">
                    <button onClick={() => onChunkChange(currentChunkIndex - 1)} disabled={currentChunkIndex === 0} className="p-2 rounded-full hover:bg-slate-700 disabled:opacity-50 transition-colors">
                    ...
                    <button onClick={handlePlayPause} className="w-12 h-12 flex items-center justify-center bg-purple-600 hover:bg-purple-700 text-white rounded-full text-2xl transition-transform transform hover:scale-105 disabled:bg-slate-600" disabled={status === 'loading'}>
                        {playButtonContent()}
                    </button>
                    <button onClick={() => onChunkChange(currentChunkIndex + 1)} disabled={currentChunkIndex >= textChunks.length - 1} className="p-2 rounded-full hover:bg-slate-700 disabled:opacity-50 transition-colors">
                 ...
                 <div className="w-20 flex justify-end items-center gap-1">
                    <button onClick={() => setIsPlaylistVisible(!isPlaylistVisible)} className={`p-2 rounded-full hover:bg-slate-700 ${isPlaylistVisible ? 'bg-slate-700 text-purple-400' : ''}`} title="Danh sách phát">
                    ...
                    <button onClick={onStop} className="p-2 rounded-full hover:bg-slate-700" title="Dừng">
                            ...
                            <li key={index}>
                                <button
                                    ...
                                    className={`w-full text-left text-sm px-3 py-2 rounded-md flex items-center justify-between transition-colors ${
                                        index === currentChunkIndex ? 'bg-purple-700/50 text-white' : 'hover:bg-slate-700/50'
                                    }`}
                                >
                                    <div className="flex items-center gap-3 truncate">
                                        {index === currentChunkIndex && status === 'playing' ? <svg className="w-4 h-4 text-purple-300" viewBox="0 0 24 24"><path d="M6 18V6l12 6-12 6z" fill="currentColor"/></svg> : <span className="w-4 text-center text-slate-400">{index + 1}</span>}
